...
def main(config):
    ...
    opt = torch.optim.SGD
    if config.get('load'):
        optimizer = SAM(model.parameters(), opt, rho=2.0, adaptive=True, lr=0.05, momentum=0.9, weight_decay=5.e-4)
        optimizer.load_state_dict(model_sv['training']['optimizer_sd'])
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [60,80])
    ...
    if ls:
        ls_loss = LabelSmoothingLoss(classes=config['n_classes'], smoothing=config['ls']['ls_value'])
    ...
    for epoch in range(start_epoch, max_epoch + 1):
        ...
        for data, label, idx in tqdm(train_loader, desc='train', leave=True):
            ...
            if als:
                # update history_label later to keep the attack and optimize loss the same
                if als_inner == False:
